...
namespace traits {
    ...
    using SmemLayout      = decltype(tile_to_shape(GMMA::Layout_MN_SW128_Atom<TB>{}, Shape<Int<TileM>, Int<TileN>>{}));
...
__global__ void copy_kernel(
    ...
    Tensor mS  = tma_load.get_tma_tensor(Shape<Int<TileM>, Int<TileN>>{});
    ...
    int tma_transaction_bytes = sizeof(make_tensor_like(tensor<0>(tAsS)));
    ...
    if (warp_idx == 0 && lane_predicate) {
        ...
        copy(tma_a.with(tma_barrier), tAgS(_), tAsA(_));
    }
...
int main() {
    ...
    Tensor mS = make_tensor(dS.data(), make_shape(m, n), LayoutRight{});
    Copy_Atom tma_load = make_tma_atom(SM90_TMA_LOAD{}, mS, SmemLayout{}(_, _), Tile<Int<TileM>, Int<TileN>>{});
